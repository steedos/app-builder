...
// export type ObjectProTableProps = {
...
export type ObjectProTableColumnProps = {
  fieldName: string
} & ProColumnType

export type ObjectProTableProps<T extends ObjectProTableColumnProps> =
  | ({
      name?: string
      objectApiName?: string
      columnFields?: T[]
...
export const getObjectProTableProColumn = (field: any, columnOption?: any) => {
...
export const ObjectProTable = observer((props: ObjectProTableProps<any>) => {
  ...
  if (object.schema && object.schema.fields) {
    forEach(
      columnFields,
      ({ fieldName, ...columnItem }: ObjectProTableColumnProps) => {
        ...
        const  proColumn = getObjectProTableProColumn(object.schema.fields[fieldName], columnOption)
        if (proColumn) {
  ...
  const request = async (
    ...
    */
    ...
    // TODO: 这里antd的request请求函数与ObjectProTable组件传入的filters,sort等格式不一样，需要转换处理
